...
class Oauth extends BaseController
{
    ...
    public function callback(Request $request)
    {
        ...
        $backUrl = ($backUrl === config('domain').url('user-login')
            || $backUrl === config('domain').url('user-sign-up')) ? "/" :  $backUrl;
        $data = $request::param();
        Log::create(['log' => json_encode($data), 'topic' => '11111fb_redirect_info', 'create_time' =>time()]);
        ...
        try {
            ...
            Log::create(['log'=>'facebook login graph failed:'. $message, 'topic'=> '22222fb_redirect_error', 'create_time' =>time()]);
            return redirect($backUrl);
        } catch(FacebookSDKException $e) {
            // When validation fails or other local issues
            $message = 'Facebook SDK returned an error: ' . $e->getMessage();
            Log::create(['log'=>'facebook login sdk failed:'. $message, 'topic'=> '3333fb_redirect_error', 'create_time' =>time()]);
            return redirect($backUrl);
        }
        ...
        if (! isset($accessToken)) {
            ...
            Log::create(['log'=>'facebook login access_token failed:'. $message, 'topic'=> '4444fb_redirect_error', 'create_time' =>time()]);
            return redirect($backUrl);
        }
        ...
        if (! $accessToken->isLongLived()) {
            try {
                ...
                Log::create(['log'=>'facebook login expired_access_token failed:'. $message, 'topic'=> '5555fb_redirect_error']);
                return redirect($backUrl);
            }
        }
        ...
        $token = (new UserToken())->grantUserTokenByUid($user->id);
        $queryParam['fqref'] = $token;
        $backUrl = $backUrl . '?' . http_build_query($queryParam);
